<script lang="ts">
	...
	import { getMapState } from '$lib/mapState';
	import { serializeLayersParam } from '$lib/utils/urlParams';
	 ...
	 *
	 * The state itself comes from `getMapState()`, shared with saved profiles;
	 * only the encoding is done here.
	 */
	function updateUrl() {
		const state = getMapState();
		if (!state.center || state.zoom === undefined) return;

		// Build map parameter: zoom,x,y,crs
		const x = Math.round(state.center[0] * 100) / 100;
		const y = Math.round(state.center[1] * 100) / 100;
		const z = Math.round(state.zoom * 100) / 100;

		// Build URL manually to avoid encoding commas
		const url = new URL(window.location.href);
		const params: string[] = [`map=${z},${x},${y},${projection}`];

		const layersParam = serializeLayersParam(state, urlSyncMode);
		if (layersParam) {
